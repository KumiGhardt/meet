...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    'https://y50hsyuzfa.execute-api.us-east-1.amazonaws.com/dev/api/token/' + encodeCode
  )
    .then((res) => {
      return res.json();
    })
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get("https://y50hsyuzfa.execute-api.us-east-1.amazonaws.com/dev/api/get-auth-url");
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = 'https://y50hsyuzfa.execute-api.us-east-1.amazonaws.com/dev/api/get-events/' + token;
